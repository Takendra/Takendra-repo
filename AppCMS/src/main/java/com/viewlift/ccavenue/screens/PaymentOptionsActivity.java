...
public class PaymentOptionsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        id_tv_text_payment.setText(getString(R.string.first_payment_text) + initialScreen.getStringExtra(AvenuesParams.AMOUNT).trim() +
                " on " + formattedDate);
...
//        //Log.v("apibaseurl",initialScreen.getStringExtra("api_base_url")) ;
    ...
    private class GetData extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... arg0) {
            ...
            try {
                vParams.add(new BasicNameValuePair(AvenuesParams.CURRENCY, initialScreen.getStringExtra(AvenuesParams.CURRENCY).trim()));
            } catch (Exception ex) {
                ex.printStackTrace();
            }

            try {
                vParams.add(new BasicNameValuePair(AvenuesParams.AMOUNT, initialScreen.getStringExtra(AvenuesParams.AMOUNT).trim()));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
                    ...
                    if(jsonRespObj!=null){
                        if(jsonRespObj.getString("payOptions")!=null){
                            ...
                            for(int i=0;i<vPayOptsArr.length();i++){
                                ...
                                payOptionList.add(new PaymentOptionDTO(vPaymentOption.getString("payOpt"), vPaymentOption.getString("payOptDesc")));//Add payment option only if it includes any card
