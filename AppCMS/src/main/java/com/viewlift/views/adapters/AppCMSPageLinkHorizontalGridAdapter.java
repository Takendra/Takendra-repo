...
public class AppCMSPageLinkHorizontalGridAdapter extends RecyclerView.Adapter<AppCMSPageLinkHorizontalGridAdapter.ViewHolder>
    ...
    public AppCMSPageLinkHorizontalGridAdapter(Context context,
        ...
        SeasonTabSelectorBus.instanceOf().getCustomSubscriber().subscribe(new Observer<CustomSubscriber>() {
            @Override
            ...
            public void onNext(CustomSubscriber customSubscriber) {
                int pos = customSubscriber.getPosition();
                if (row_index != pos && pos != -1) {
                    row_index = pos;
                    notifyDataSetChanged();
                    if (!customSubscriber.isHorizontalScroll()) {
                        ((LinearLayoutManager) recyclerView.getLayoutManager()).scrollToPositionWithOffset(row_index, 0);
                    } else {
                        ((LinearLayoutManager) scheduleWeekRecyclerView.getLayoutManager()).scrollToPositionWithOffset(row_index, 0);
                    }
                }
            }
    ...
    @Override
    public AppCMSPageLinkHorizontalGridAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        scheduleWeekRecyclerView = pageView.findViewById(R.id.scheduleWeekViewGrid);
        return new AppCMSPageLinkHorizontalGridAdapter.ViewHolder(view);
    }
   /* @Override
    ...
    @Override
    public void onBindViewHolder(AppCMSPageLinkHorizontalGridAdapter.ViewHolder holder, final int position) {
        ...
        for (int i = 0; i < holder.componentView.getChildItems().size(); i++) {
            ...
            if (itemContainer.getComponent().getKey() != null) {
                ...
                } else if (itemContainer.getComponent().getKey().contains(mContext.getString(R.string.app_cms_page_api_title_key))) {
                    itemLine = itemContainer.getChildView();
                    background = itemContainer.getChildView();
                }
            }
        ...
        if (background != null) {
            if (row_index == position) {
                background.setBackgroundColor(Color.parseColor(appCMSPresenter.getSettings().getStyle().getActiveBackgroundColor()));
                //background.setBackgroundColor(ContextCompat.getColor(mContext, R.color.color_dark_blue));
            } else if (position == 0 && row_index == -1) {
                //background.setBackgroundColor(ContextCompat.getColor(mContext, R.color.color_dark_blue));
                background.setBackgroundColor(Color.parseColor(appCMSPresenter.getSettings().getStyle().getActiveBackgroundColor()));

            } else {
                background.setBackgroundColor(Color.parseColor(appCMSPresenter.getSettings().getStyle().getBackgroundColor()));
                // background.setBackgroundColor(ContextCompat.getColor(mContext, R.color.color_light_blue));
            }
        }
    }
            ...
            onClickHandler = new CollectionGridItemView.OnClickHandler() {
                ...
                public void click(CollectionGridItemView collectionGridItemView,
                    ...
                    if (jsonValueKeyMap.get(component.getKey()) == AppCMSUIKeyType.PAGE_WEEK_SCHEDULE_DATE_GRID_KEY ||
                            jsonValueKeyMap.get(component.getKey()) == AppCMSUIKeyType.PAGE_LINK_MODULE_HORIZONTAL_GRID_KEY) {
                        appCMSPresenter.setItemViewClicked(true);
                        row_index = clickPosition;
                        Utils.catagoryPosition = clickPosition;
                        notifyDataSetChanged();
