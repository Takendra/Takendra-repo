...
public class AppCMSUserPagesAdapter extends RecyclerView.Adapter<AppCMSUserPagesAdapter.ViewHolder>
    ...
    protected ConstraintViewCreator constraintViewCreator;
    ...
    ConstraintCollectionGridItemView view = null;
    ...
    public AppCMSUserPagesAdapter(Context context,
                                  ...
                                  int defaultHeight,
                                  String viewType,
                                  AppCMSAndroidModules appCMSAndroidModules, String blockName,PageView pageView) {
    ...
    private void sortData() {
        if (adapterData != null) {
            if (isWatchlistPage || isDonwloadPage || isFollowPage) {
                sortByAddedDate();
    ...
    private void sortByUpdateDate() {
        try {
            Collections.sort(adapterData, (o1, o2) -> Double.compare((Double) o1.getUpdateDate(),
    ...
    private void downloadView(ContentDatum contentDatum, ConstraintCollectionGridItemView componentView, int position) {
        ...
        if (videoSize != null && deleteDownloadButton != null && thumbnailImage != null && contentDatum.isDRMEnabled()) {
            ...
            View.OnClickListener deleteClickListener =  new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    deleteDownloadVideo(contentDatum, position);
                }
            };
...
//                    sizeValue = appCMSPresenter.getDownloadedFileSize(appCMSPresenter.getAppPreference().getofflineExoSize(contentDatum.getGist().getId()));
                ...
                deleteDownloadButton.setImageBitmap(null);
                deleteDownloadButton.setBackground(null);
            }
            else {
...
//    }
    ...
    public int getItemCount() {
        if (isHistoryPage || isDonwloadPage || isWatchlistPage || isLibraryPage || isFollowPage) {
            ...
            onClickHandler = new ConstraintCollectionGridItemView.OnClickHandler() {
                ...
                public void click(ConstraintCollectionGridItemView collectionGridItemView, Component childComponent, ContentDatum data, int clickPosition) {
                    if (isClickable) {
                        ...
                        if (action.equalsIgnoreCase(mContext.getString(R.string.app_cms_hide_show_details_action))) {
                            ...
                            if (childcollectionGridItemView != null) {
                                try {
                                    ...
                                    RecyclerView.LayoutManager  layoutManager=childcollectionGridItemView.getLayoutManager();
                        ...
                        else if (data.getGist() != null && data.getGist().getContentType() != null && data.getGist().getContentType().equalsIgnoreCase(mContext.getString(R.string.content_type_event))) {
                            ...
                            if (action.contains(deleteSingleItemWatchlistAction)) {
                                deleteWatchlist(data, clickPosition);
                                return;
                            }
                            if (action.contains(deleteSingleItemHistoryAction)) {
                                deletehistorylist(adapterData.get(position), position);
                                return;
                            }
...
//                                                appCMSPresenter.sendCloseOthersAction(null, true, false);
                                ...
                                if (data.getGist().getId() != null && data.getSeriesData() != null &&
                                        data.getSeriesData().size() > 0 && data.getSeriesData().get(0).getGist() != null && data.getSeriesData().get(0).getGist().getPermalink() != null) {
...
//                return;
//            }
//        } else if (hlsUrl != null && !new File(hlsUrl.substring(7)).exists()) {
//            appCMSPresenter.showDialog(AppCMSPresenter.DialogType.DOWNLOAD_INCOMPLETE,
//                    appCMSPresenter.getLanguageResourcesFile().getUIresource(mContext.getString(R.string.app_cms_download_file_error_message)),
//                    false,
//                    null,
...
//        notifyItemRemoved(position);
        ...
        if (isWatchlistPage)
            deleteWatchlist(adapterData.get(position), position);
    }

    void deleteWatchlist(ContentDatum data, int position) {
        String dialogTitle = "";
    ...
    void deletehistorylist(ContentDatum data, int position) {
        String dialogTitle = "";
