...
public class CountryCodeSpinnerAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = (LayoutInflater.from(context)).inflate(R.layout.element_country_code, null);
        ...
        try {
            ...
            holder.separatorView.setBackgroundColor(Color.parseColor(appCMSPresenter.getAppTextColor()));
            holder.countryCode.setTextColor(Color.parseColor(appCMSPresenter.getAppTextColor()));
        }catch (Exception e){
        }
        holder.countryCode.setText("+" + m_Codes[position]);
        return convertView;
    }
    ...
    public static int getCountryCodeIndex(String countryCode) {
        for (int i = 0; i < m_Codes.length; ++i) {
            if (m_Codes[i].equals(countryCode))
                return i;
        }
        return -1;
    }
